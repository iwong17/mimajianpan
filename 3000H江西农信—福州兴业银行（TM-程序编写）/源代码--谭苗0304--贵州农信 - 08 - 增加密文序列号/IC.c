...
int IC_Apdu_T0(unsigned char *cmd_send,unsigned short cmd_len,unsigned char *res)
{
	...
	while(1)
	{
		...
		if(res[0]==INS)
		{
			if(cmd_len!=5)
			{
				if(IC_CommandSend(cmd_send+count,cmd_len-5))return -1;
				cmd_len=5;
				continue;
			}
			else
			{
				res_len=0;
				if(IC_Response(res,2+templen))return -2;
				res_len=templen+2;
			}
		}
		else if(res[0]==~INS+1)
		{
			if(cmd_len!=5)
			{
				if(IC_CommandSend(cmd_send+count,1))return -1;
				cmd_len--;
				count++;
			}
			continue;
		...
		{
			if(IC_Response(res+res_len+1,1))return -2;
			res_len++;
			tempdata[4]=res[res_len];
			if(IC_CommandSend(tempdata,5))return -1;
			templen=tempdata[4];
			INS = tempdata[1];
			continue;
		}
		else if(res[0]==0x6c)
		{
			...
			templen = res[res_len];
			if(IC_CommandSend(cmd_send,5))return -1;
			continue;
		}
		else
		{
